...
<html>
	...
	<body>
				...
				<div class='search clearfix'>
					<h3>招标信息管理</h3>
					<span class="datum">招标信息管理>>招标信息记录</span>
					<form name="reg_testdate" action="bidInfo_list2.html" method="post">
				...
				<div class="ifamelayout">
					<table class="querytable" border="1px" cellspacing="0" cellpadding="3px" style="text-align: center;">
						...
						<tr>
							...
							<td><c:choose>
							<c:when test="${model.bidtype == '1'}">招投标</c:when>
							<c:otherwise>行业动态</c:otherwise></c:choose>
							</td>
							<td>${model.createTimeStr}</td>
							<td>
								<span class="add">
									<a href="bidInfo_modify.html?id=${model.id}" style="color:white">修改</a>
								</span>
							&nbsp;&nbsp;
								<span class="del">
									<a href="javascript:if(confirm('确实要删除${model.title}吗?'))location='bidInfo_del.html?id=${model.id}'" style="color:white">删除</a>
								</span>
							</td>
						</tr>
						</c:forEach>
