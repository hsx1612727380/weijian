		...
		<div class="detail">
			...
			<div class="detailright">
				<form action="updateInfo.html" method="post">
					...
					<input name="code" type="hidden" value="${materialModel.code}">
					<div>
						<label>供应商名称</label> <input name="name" type="text"  value="${materialModel.name}" placeholder="必填">
					</div>
					<div>
						<label>材料</label> <input name="shopName" type="text"  value="${materialModel.shopName}" placeholder="必填">
					</div>
					<div>
						<label>统一社会信用代码</label> <input name="licence" type="text" value="${materialModel.licence}" placeholder="填写这项可以提高可信度哦！" />
					</div>
				<%-- 	<div>
						<label>成立日期</label>
						<input type="text" name="establishDate" member.birth value="${materialModel.establishDate}" placeholder="格式示例：20160802"/>
					</div> --%>
					<div>
						<label>成立日期</label>
						<input name="establishDate" type="text" class="laydate-icon" id="demo" value="${materialModel.establishDate}"/>
					</div>
					<div>
						<label>注册资金（万）</label> <input name="regcapital" type="text" value="${materialModel.regcapital}"  placeholder="注册资金以万元计"/>
					</div>
					<div>
						<label>注册地址</label> 
						<select id="province" onchange="setCity(this.value);getArea();" name="province" >
                              <option value="${materialModel.province}">--请选择省份--</option>
                        </select>
                    </div>
					<div>
                        <label>城市</label> 
                        <select id="city" onchange="setCounty(this.value);getArea();" name="city" >
                              <option value="${materialModel.city}" selected="selected">--请选择城市--</option>
                        </select>
                    </div>
					<div>
                        <label>街道</label> 
                        <input name=street type="text" value="${materialModel.street}"  placeholder="具体街道地址"/>
					</div>
					<div>
						<label>联系人</label> <input name="leaderName" type="text" value="${materialModel.leaderName}"  />
					</div>
					<div>
						<label>电话</label> ${materialModel.userId}
					</div>
					<div>
						<label>诚信度</label> 
						<input type="hidden" name="reliableStar" value="${materialModel.reliableStar}"/>
