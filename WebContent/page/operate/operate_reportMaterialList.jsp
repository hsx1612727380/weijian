...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			function operate_reportBenMaterialListToExcel() {
				var year = $("#year").val().trim();
				var month = $("#month").val().trim();
				var projectId = $("#projectId").val().trim();
				window.location.href="operate_reportBenMaterialListToExcel.html?year="+year+"&month="+month+"&projectId="+projectId;
			}
			function operate_reportLaoMaterialListToExcel() {
				var year = $("#year").val().trim();
				var month = $("#month").val().trim();
				var projectId = $("#projectId").val().trim();
				window.location.href="operate_reportLaoMaterialListToExcel.html?year="+year+"&month="+month+"&projectId="+projectId;
			}
			function operate_reportZhuMaterialListToExcel() {
				var year = $("#year").val().trim();
				var month = $("#month").val().trim();
				var projectId = $("#projectId").val().trim();
				window.location.href="operate_reportZhuMaterialListToExcel.html?year="+year+"&month="+month+"&projectId="+projectId;
			}
		</script>
	</head>
			...
			<table border="0" cellspacing="1" cellpadding="0">
				...
				<tr>
					...
					<td style="text-align: left;" colspan="20">
						&nbsp;&nbsp;本班组材料费&nbsp;&nbsp;&nbsp;
						<span class="items">
							<div><a href="operate_reportMEAdd.html?projectId=${projectModel.id }&teamtype=1&type=2" style="color:white;">新增</a></div>
							&nbsp;&nbsp;&nbsp;
							<div style="margin-top: 11px;" id="toExcel" onclick="return operate_reportBenMaterialListToExcel();">导出EXCEL</div>
							&nbsp;&nbsp;&nbsp;
							<a class="chartadd" href="operate_reportMEDownloadExcel.html" style="color: white; width: 100px;">EXCEL模板下载</a>
							<form action="operate_reportMEImportExcel.html?projectId=${projectModel.id }&flag=1&year=${year}&month=${month}&teamtype=1&type=2" method="post" enctype="multipart/form-data" style='display:inline-block'>
				...
				<c:forEach items="${benCmdSummModels}" var="benCmdSummModel" varStatus="status">
					<tr>
						...
						<td><a href="operate_reportMEDetailList.html?projectId=${projectModel.id }&cmdId=${benCmdSummModel.id}&tId=${benCmdSummModel.tId}&teamtype=1&type=2&year=${year}&month=${month}&result=0" style="color:orange">${benCmdSummModel.tName}</a></td>
				...
				<tr>
					...
					<td style="text-align: left;" colspan="20">
						劳务班组材料费&nbsp;
						<span class="items">
							<div><a href="operate_reportMEAdd.html?projectId=${projectModel.id }&teamtype=2&type=2" style="color:white;">新增</a></div>
							&nbsp;&nbsp;&nbsp;
							<div style="margin-top: 11px;" id="toExcel" onclick="return operate_reportLaoMaterialListToExcel();">导出EXCEL</div>
							&nbsp;&nbsp;&nbsp;
							<a class="chartadd" href="operate_reportMEDownloadExcel.html" style="color: white; width: 100px;">EXCEL模板下载</a>
							<form action="operate_reportMEImportExcel.html?projectId=${projectModel.id }&flag=1&year=${year}&month=${month}&teamtype=2&type=2" method="post" enctype="multipart/form-data" style='display:inline-block'>
								<i class="chartadd" style='position:relative;margin-right:3px'>
									导入Execl
									<input id="excelReportMEInput" name="fileInput" type="file" style="position:absolute;width:100%;height:100%;left:0;top:0;opacity:0" />
								</i>
				...
				<c:forEach items="${laoCmdSummModels}" var="laoCmdSummModel" varStatus="status">
					<tr>
						<td>${status.index + 1}</td>
						<td><a href="operate_reportMEDetailList.html?projectId=${projectModel.id }&cmdId=${laoCmdSummModel.id}&tId=${laoCmdSummModel.tId}&teamtype=2&type=2&year=${year}&month=${month}&result=0" style="color:green">${laoCmdSummModel.tName}</a></td>
				...
				<tr>
					...
					<td style="text-align: left;" colspan="20">
						专业班组材料费&nbsp;
						<span class="items">
							<div><a href="operate_reportMEAdd.html?projectId=${projectModel.id }&teamtype=3&type=2" style="color:white;">新增</a></div>
							&nbsp;&nbsp;&nbsp;
							<div style="margin-top: 11px;" id="toExcel" onclick="return operate_reportZhuMaterialListToExcel();">导出EXCEL</div>
							&nbsp;&nbsp;&nbsp;
							<a class="chartadd" href="operate_reportMEDownloadExcel.html" style="color: white; width: 100px;">EXCEL模板下载</a>
							<form action="operate_reportMEImportExcel.html?projectId=${projectModel.id }&flag=1&year=${year}&month=${month}&teamtype=3&type=2" method="post" enctype="multipart/form-data" style='display:inline-block'>
								<i class="chartadd" style='position:relative;margin-right:3px'>
									导入Execl
									<input id="excelReportMEInput" name="fileInput" type="file" style="position:absolute;width:100%;height:100%;left:0;top:0;opacity:0" />
								</i>
				...
				<c:forEach items="${zhuCmdSummModels}" var="zhuCmdSummModel" varStatus="status"> 
					<tr>
						<td>${status.index + 1}</td>
						<td><a href="operate_reportMEDetailList.html?projectId=${projectModel.id }&cmdId=${zhuCmdSummModel.id}&tId=${zhuCmdSummModel.tId}&teamtype=3&type=2&year=${year}&month=${month}&result=0" style="color:red">${zhuCmdSummModel.tName}</a></td>
