...
<html>
	...
	<body>
			...
			<ul class="info_l">
				...
				<li>
					<div data-index="${flag }">
						收件箱
						<div>
							<a href="operate_message.html?isRead=1&userId=${projectModel.userId }">已读<c:if test="${readMessageCount > 0 }">(${readMessageCount })</c:if></a>
							<a href="operate_message.html?isRead=0&userId=${projectModel.userId }">未读<c:if test="${unReadMessageCount > 0 }">(${unReadMessageCount })</c:if></a>
						</div>
					</div>
				</li>
				<li>
					<div data-index="${flag }">
						公告
						<div>
							<a href="operate_notice.html?isRead=1&userId=${projectModel.userId }">已读<c:if test="${readNoticeCount > 0 }">(${readNoticeCount })</c:if></a>
							<a href="operate_notice.html?isRead=0&userId=${projectModel.userId }">未读<c:if test="${unReadNoticeCount > 0 }">(${unReadNoticeCount })</c:if></a>
						</div>
					</div>
				</li>
			</ul>
						...
						<c:forEach items="${unReadMessageModels }" var="unReadMessageModel">
							<li>
								...
								<p class="info_mess">${unReadMessageModel.createTimeStr }
									<a href="inviteTeamProjectForm.html" class="mess_notice">
		...
		function updateMessageIsRead(obj) {
			$.ajax({
			    type : 'post',
			    url : 'operate_messageIsRead.html',
			    dataType : 'text',
			    data : {"messageId" : obj},
			    success : function (date) {
			  		// 什么也不做，目的就是通过后台修改一个字段
			    },
