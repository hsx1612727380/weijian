...
<html>
	...
	<body>
				...
				<div class='search clearfix'>
					<h3>材料商管理</h3>
					<span class="datum">材料商管理>>材料商列表</span>
					<form action="material_list2.html" method="post">
					<table border="0px" cellspacing="0px" cellpadding="0px">
						<tr>
							<td><label for="gender">材料商代码:</label></td>
				...
				<div class="ifamelayout">
					<table class="querytable" border="1px" cellspacing="0" cellpadding="3px" style="text-align: center;">
						...
						<c:forEach items="${mlist}" var="model"> 
						<tr>
							<td><a href="merchant_form.html?id=${model.id}&code=${model.code}&type=1" style="color:blue">${model.name}</a></td>
							<td>${model.code}</td>
							...
							<td><c:choose>
							   ...
							   <c:otherwise> 
							    <span class="del">
									<a href="javascript:if(confirm('确实要审核通过${model.name}吗?'))location='material_pass.html?id=${model.id}'" style="color:white">审核</a>
								</span> 
							   </c:otherwise>
							</c:choose></td>
							<td>
								<span class="items">
									<a href="merchant_form.html?id=${model.id}&code=${model.code}&type=1" style="color:white">查看</a>
								</span>
								
								
								<span class="add">
									<a href="material_modify.html?id=${model.id}" style="color:white">修改</a>
								</span>
							&nbsp;&nbsp;
								<span class="del">
									<a href="javascript:if(confirm('确实要删除${model.name}吗?'))location='material_del.html?id=${model.id}'" style="color:white">删除</a>
								</span>
							</td>
						</tr>
						</c:forEach>
