...
<html>
	...
	<body>
<form action="job_person_list.html" method="post">
				<div class='search clearfix'>
					<h3>求职</h3>
					<span class="datum">求职>>个人求职</span>
				...
				<div class="ifamelayout">
						<span class="items"><a href="job_person_add.html">添加</a></span>
						...
						<tr>
							...
							<td>
								<span class="add">
									...
									<a href="javascript:if(confirm('确实要关闭${tmodel.title}吗?'))location='job_person_close.html?id=${tmodel.id}&status=0'" style="color:white">关闭</a>
									</c:if>
									<c:if test="${tmodel.status=='0'}">
									<a href="javascript:if(confirm('确实要开启${tmodel.title}吗?'))location='job_person_close.html?id=${tmodel.id}&status=1'" style="color:white">开启</a>
									</c:if>
								</span>
								<span class="del">
									<a href="javascript:if(confirm('确实要删除${tmodel.title}吗?'))location='job_person_del.html?id=${tmodel.id}'" style="color:white">删除</a>
								</span>
							</td>
						</tr>
					</c:forEach> 
