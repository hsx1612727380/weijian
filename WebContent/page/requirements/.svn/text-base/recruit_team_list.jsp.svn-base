...
<html>
	...
	<body>
	<form action="recruit_team_list.html" method="post">
				<div class='search clearfix'>
					<h3>招聘</h3>
					<span class="datum">招聘>>招聘班组</span>
				...
				<div class="ifamelayout">
						<span class="items"><a href="recruit_team_add.html">添加</a></span>
						...
						<tr>
							...
							<td>
								<span class="add">
									...
									<a href="javascript:if(confirm('确实要关闭${model.title}吗?'))location='recruit_team_close.html?id=${model.id}&status=0'" style="color:white">关闭</a>
									</c:if>
									<c:if test="${model.status=='0'}">
									<a href="javascript:if(confirm('确实要开启${model.title}吗?'))location='recruit_team_close.html?id=${model.id}&status=1'" style="color:white">开启</a>
									</c:if>
								</span>
								<span class="del">
									<a href="javascript:if(confirm('确实要删除${model.title}吗?'))location='recruit_team_del.html?id=${model.id}'" style="color:white">删除</a>
								</span>
							</td>
						</tr>
					</c:forEach> 
