...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
				...
				<div class='search clearfix'>
						...
						<form action="payrollrecord_project2.html"  name="reg_testdate" method="post" onSubmit="return submitForm()">
				...
				<table class="querytable" border="1px" cellspacing="0" cellpadding="3px" style="text-align: center;">
					...
					<c:forEach items="${tolist}" var="model" >
					<tr>
						 
						<td><a href="payrollrecord_project.html?pCode=${pCode}&code=${model.code}" style="color:blue">${model.code}</a></td>
						<td>${model.name}</td>
						<td>${model.totalSalary}</td>
						<td>${model.totalRealSalary}</td>
						<td>${model.totalNoSalary}</td>
				...
				<div class="ifamelayout">
					<table class="querytable" border="1px" cellspacing="0" cellpadding="3px" style="text-align: center;">
						...
						<c:forEach items="${pplist}" var="model"> 
						<tr>
							<td>${model.tName}</td>
							<td>${model.name}</td>
							<td>${model.userId}</td>
