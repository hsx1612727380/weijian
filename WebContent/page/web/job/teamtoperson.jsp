...
<html>
	...
	<body>
		...
		<form action="teamtoperson.html" name="form1" method="post" class="struck">
		<div class="title">
			<div class="content">
				<span>个人求职</span>
			</div>
		...
		<div class="content personal">
			...
			<table border="0" cellspacing="1" cellpadding="0">
				...
				<tr >
					<td><a href="job_person.html?userId=${model.userModel.userId}&id=${model.id}" style="color: #4488FF;">${model.userModel.userName}</a></td>
					<td><c:if test="${model.userModel.userSex=='1'}">男</c:if>
					<c:if test="${model.userModel.userSex=='2'}">女</c:if></td>
					<td><a href="job_person.html?userId=${model.userModel.userId}&id=${model.id}" style="color: #4488FF;">${model.userModel.userId}</a></td>
					<td>${model.skillBigTypeName}</td>
					<td>${model.skillSmallTypeName}</td>
					<td>${model.createTimeStr}</td>
					<td>${model.provinceStr}${model.cityStr}</td>
					<td><c:forEach begin="1" end="${model.userModel.reliableStar}">
								   <span><img src="resource/images/u662.png"/></span>
								</c:forEach>
								<c:forEach begin="1" end="${model.userModel.noreliableStar}">
					...
					<td>
						...
						<a
							href="javascript:if(confirm('确实要邀请${model.userModel.userName}吗?'))location='teamapplyperson.html?id=${model.userModel.userId}'"
