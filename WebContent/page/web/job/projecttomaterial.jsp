...
<html>
	...
	<body>
		...
		<form action="projecttomaterial.html" name="form1" method="post" class="struck">
		<div class="title">
			<div class="content">
				<span>材料供应</span>
			</div>
		...
		<div class="content personal">
			...
			<table border="0" cellspacing="1" cellpadding="0">
				...
				<tr >
					<td><a href="job_material.html?mId=${model.rId}&id=${model.id}" style="color: #4488FF;">${model.materialModel.name}</a></td>
					<td><a href="web_person_info.html?userId=${model.materialModel.userId}" style="color: #4488FF;">${model.materialModel.leaderName}</a></td>
					<td>材料班组</td>
					<td>${model.shopName}</td>
					<td>${model.provinceStr}${model.cityStr}</td>
					<td><c:forEach begin="1" end="${model.materialModel.reliableStar}">
								   <span><img src="resource/images/u662.png"/></span>
								</c:forEach>
								<c:forEach begin="1" end="${model.materialModel.noreliableStar}">
					...
					<td>
						...
						<a
							href="javascript:if(confirm('确实要邀请${model.name}吗?'))location='projectapplymaterail.html?rId=${model.rId}'"
