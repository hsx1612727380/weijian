...
<body>
		...
		<ul class="info_l">
			...
			<li>
				<div data-index="${flag }">
					收件箱
					<div>
						<a href="company_message.html?isRead=1">已读<c:if test="${readMessageCount > 0 }">(${readMessageCount })</c:if></a>
						<a href="company_message.html?isRead=0">未读<c:if test="${unReadMessageCount > 0 }">(${unReadMessageCount })</c:if></a>
					</div>
				</div>
			</li>
			<li>
				<div data-index="${flag }">
					公告
					<div>
						<a href="company_notice.html?isRead=1">已读<c:if test="${readNoticeCount > 0 }">(${readNoticeCount })</c:if></a>
						<a href="company_notice.html?isRead=0">未读<c:if test="${unReadNoticeCount > 0 }">(${unReadNoticeCount })</c:if></a>
					</div>
				</div>
			</li>
		</ul>
						...
						<li>
							...
							<p class="info_mess">${unReadNoticeModel.createTimeStr }
								<a href="javascript://" class="mess_notice">
									...
									<input type="hidden" value="${companyModel.id }"/>
									查看
								</a>
							</p>
						</li>
	...
	<script type="text/javascript">
		...
		$('.mess_notice').on('click', function() {
			...
			var cId = $(this).children()[1].value;
			getNoticeInfo(nId, cId); 
			$('.mask').show();
			$('.notice').show();
		})
