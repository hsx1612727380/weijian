		...
		<div class="detail">
			...
			<div class="detailright">
				...
				<table class="invite" border="0" cellspacing="1" cellpadding="5">
					...
					<c:forEach items="${projectVoList}" varStatus="status" var="projectVo">
					<tr>
						<td>${status.index+1}</td>
						<td>${projectVo.projectModel.name}</td>
						<td>${projectVo.projectModel.provinceChn}${projectVo.projectModel.cityChn}
							${projectVo.projectModel.street}</td>
						<td>
							<input type="hidden" id="visitAddress" value="<%=IMageUploadInfo.VISITADDRESS.getValue()%>" />
							<input type="hidden" id="materialPath" value="<%=IMageUploadInfo.EQUIPMENTPROJECTPATH.value%>" />
						</td>
						<td>
				...
				<table class="invite" border="0" cellspacing="1" cellpadding="5">
					...
					<c:forEach items="${projectVoListPast}" var="projectVo">
					<tr>
						<td>${status.index+1}</td>
						<td>${projectVo.projectModel.name}</td>
