...
<body>
		...
		<ul class="info_l">
			...
			<li>
				<div data-index="${flag }">
					收件箱
					<div>
						<a href="pt_message.html?isRead=1">已读<c:if test="${readMessageCount > 0 }">(${readMessageCount })</c:if></a>
						<a href="pt_message.html?isRead=0">未读<c:if test="${unReadMessageCount > 0 }">(${unReadMessageCount })</c:if></a>
					</div>
				</div>
			</li>
			<li>
				<div data-index="${flag }">
					公告
					<div>
						<a href="pt_notice.html?isRead=1">已读<c:if test="${readNoticeCount > 0 }">(${readNoticeCount })</c:if></a>
						<a href="pt_notice.html?isRead=0">未读<c:if test="${unReadNoticeCount > 0 }">(${unReadNoticeCount })</c:if></a>
					</div>
				</div>
			</li>
		</ul>
							...
							<p class="info_mess">${unReadNoticeModel.createTimeStr }
								<a href="javascript://" class="mess_notice">
									...
									<c:if test="${isLeader == '1'}">
										<input type="hidden" value="${teamModel.id }"/>
									</c:if>
									<c:if test="${isLeader == '0'}">
										<input type="hidden" value="${personModel.id }"/>
									</c:if>
									<c:if test="${isLeader == '2'}">
										<input type="hidden" value="${personModel.id }"/>
									</c:if>
									查看
								</a>
	...
	<script type="text/javascript">
		...
		$('.mess_notice').on('click', function() {
			...
			var ptId = $(this).children()[1].value;
			var isLeader = $("#isLeader").val().trim();
			getNoticeInfo(nId, ptId, isLeader); 
			$('.mask').show();
			$('.notice').show();
		})
		...
		function getNoticeInfo(obj, obj2, obj3) {
			$.ajax({
			    type : 'post',
			    url : 'pt_noticeInfo.html',
			    dataType : 'text',
			    data : {"nId" : obj,
			    	"ptId" : obj2,
			    	"isLeader" : obj3},
